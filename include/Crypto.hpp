...
public:
    ...
    static std::string hash_password(const std::string& password, const std::string& salt) {
        ...
        if (crypto_pwhash(hashed_password, sizeof(hashed_password),
        ...
        std::string hashed_password_str(reinterpret_cast<const char*>(hashed_password));
    ...
    static std::string generate_auth_token(size_t bytes) {
        ...
        if (sodium_bin2hex(&hex_str[0], hex_str.length() + 1,
                          random_bytes, sizeof(random_bytes)) == nullptr) {
